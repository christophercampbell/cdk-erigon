...
func FuzzEVMRunner(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, inputSender, inputReceiver, inputCode, inputInput []byte, initialGas, inputPrice, inputValue uint64, inputCreateContract bool) {
		...
		err := runEVM(t, inputSender, inputReceiver, inputCode, inputInput, initialGas, inputPrice, inputValue, inputCreateContract)
...
func sanitizeHex(input []byte) []byte {
	hexString := bytes.TrimSpace(input)
...
func getTestAllocs() types.GenesisAlloc {
	...
	for _, acc := range accounts {
		ga[acc] = types.GenesisAccount{Balance: initBalance}
	}
	return ga
...
func runEVM(t *testing.T, inputSender, inputReceiver, inputCode, inputInput []byte, initialGas, inputPrice, inputValue uint64, inputCreateContract bool) error {
	...
	// FIXME: Some are coming in improperly encoded?
	// normalize to hex
	hexStr := string(hexcode)
	if !hex.IsValid(hexStr) {
		hexStr = hex.EncodeToHex(hexcode)
	}
